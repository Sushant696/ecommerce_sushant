    ...
    <>
      ...
      <div>
        ...
        <div  className="" >
          ...
          {state.cart.map((item) => (
            <div className="flex justify-between m-4 text-center" key={item.id} >
              <div className="border-[#444] border-[3px] p-3 w-[11rem] text-center ">
                <img className="h-[6rem] w-[9rem]" src={`${item.thumbnail}`} alt="my_thumbnail" />
              <h2 className="font-semibold text-[1.2rem]">{item.title}</h2>
              </div>
              <div>
                <h2 className="font-semibold text-[1rem]">${item.price}</h2>
              <button  className="font-semibold text-[#fff] self-center p-1 rounded bg-[#900]" onClick={() => removeFromCart(item)}>Remove</button>
              </div>
            </div>
          ))}
        </div>
